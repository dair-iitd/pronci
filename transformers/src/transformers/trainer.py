...
class Trainer:
    ...
    def evaluation_loop(
        ...
        labels_host = None
        ## ksk
        none_labels_host = None
        # losses/preds/labels on CPU (final containers)
        all_losses = None
        all_preds = None
        all_labels = None
        ## ksk
        all_none_labels = None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            ## ksk: added none_labels
            loss, logits, labels, none_labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if none_labels is not None:
                none_labels = self._pad_across_processes(none_labels)
                none_labels = self._nested_gather(none_labels)
                none_labels_host = none_labels if none_labels_host is None else nested_concat(none_labels_host, none_labels, padding_index=-100)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                ## ksk
                if none_labels_host is not None:
                    none_labels = nested_numpify(none_labels_host)
                    all_none_labels = (
                        none_labels if all_none_labels is None else nested_concat(all_none_labels, none_labels, padding_index=-100)
                    )
        ...
        if none_labels_host is not None:
            none_labels = nested_numpify(none_labels_host)
            all_none_labels = none_labels if all_none_labels is None else nested_concat(all_none_labels, none_labels, padding_index=-100)
        ...
        ## ksk
        if all_none_labels is not None:
            all_none_labels = nested_truncate(all_none_labels, num_samples)

        # Metrics!
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            ## ksk added cls_labels
            metrics = self.compute_metrics(EvalPrediction(predictions=(all_preds, all_none_labels), label_ids=all_labels))
        else:
            metrics = {}
